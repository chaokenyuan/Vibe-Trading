...
本 change 未實作的 9 條規則（`SignalFreshnessRule`、`SymbolWhitelistRule`、`StrategyPausedRule`、`PerOrderSizeCap`、`StrategyBudgetCap`、`SymbolConcentrationCap`、`ThrottleScaler`、`PriceSanityCheck`、`CapitalReservation`）SHALL 提供 Protocol 契約 stub，呼叫時 SHALL 拋出 `NotImplementedError`，但其簽名與 docstring SHALL 完整定義。

#### Scenario: 未實作規則拋出 NotImplementedError

- **WHEN** RuleEngine 嘗試評估 SymbolWhitelistRule（未實作）
- **THEN** 該規則 SHALL 拋出 `NotImplementedError`，附訊息 "rule not implemented: SymbolWhitelistRule, see openspec/changes/<future-change>"

#### Scenario: 未實作規則的契約已凍結

- **WHEN** 開發者讀 `risk/rules/_stubs.py` 或對應檔
- **THEN** 每條 stub 規則的 `evaluate(ctx) -> RuleVerdict` 簽名與 docstring 已存在，後續 change 僅需填入內部邏輯
